...
@Component({
  selector: 'account-setup-list',
  standalone: false,
  templateUrl: './account-setup-list.html',
  styleUrls:  ['./account-setup-list.css','../../../common.css']
})
export class AccountSetupList {
  ...
  currPage: number = 1;
  selectedAccountSetup: number=0;
  filterAccountSetup = new AccountSetupModel();
  filteredRows: any[] = [];  
  accountSetupPagingResponse: Pagination = new Pagination();
  startCount:any = 0;
  totalPages : number = 0;
  ...
  endCount: number=0;
   ...
   constructor(public accountSetupService:AccountSetupService,public commonService:CommonService,public endPointService:EndPointService,public userAccessService:UserAccessService,
            private cdRef: ChangeDetectorRef) {
    this.gridHeight = this.endPointService.GridHeight;

    this.subscription.push(this.accountSetupService.loadList.subscribe(async data => {
      if(data){
        if (data.length !== 0 ) {
          ...
          this.accountSetupPagingResponse.TotalItems = this.rows.length;
    ...
    this.subscription.push(this.accountSetupService.disableGrid.subscribe(data=>{
      this.disableGrid = data;
    }));

    this.subscription.push(this.accountSetupService.ControlsEnableAndDisable.subscribe(data=>{
      if(data){
        this.ControlsEnableAndDisable();
      }
    }));

    this.subscription.push(this.accountSetupService.ngOnInit.subscribe(data=>{
      if(data){
        this.accountSetupService.GetAccountSetupList(1);
        this.disableGrid = false;
        this.accountSetupService.btnClick.next('');
        this.accountSetupService.cancelClick.next(true);
      }
    }));
  ...
  async ngOnInit() {
    this.filterAccountSetup = new AccountSetupModel();
    await this.accountSetupService.GetAccountSetupList(1);
  }
  ...
  ngOnDestroy(): void {
    ...
    if(rowIndex+1!=this.controls.pageSize){
      if (event.type === 'keydown' && (event.event.code === 'ArrowDown'))
      {
        ...
        this.selectedAccountSetup = 1 + rowIndex;
        this.viewDetails(this.rows[this.selectedAccountSetup]);
        this.accountSetupService.clickedAccount.next(this.rows[this.selectedAccountSetup]);
      } 
      else if (event.type === 'keydown' && (event.event.code === 'ArrowUp'))
      {
        this.selectedAccountSetup = rowIndex - 1;
        this.viewDetails(this.rows[this.selectedAccountSetup]);
        this.accountSetupService.clickedAccount.next(this.rows[this.selectedAccountSetup]);
      }
      ...
      {
        let rowItem = event.row;
        this.selectedAccountSetup = this.rows.indexOf(rowItem);
  ...
  updateFilter() {
    const f = this.filterAccountSetup;
    this.filteredRows = this.allRows.filter(row =>
      (!f.companY_CODE || row.companY_CODE?.toLowerCase().toString().includes(f.companY_CODE.toLowerCase().toString()))
    );
    ...
    this.accountSetupService.clickedAccount.next(this.rows[0]);
  }
  ...
  ControlsEnableAndDisable() {
    ...
    this.userAccessService.CheckUserAccess(this.accountSetupService.FormName, this.accountSetupService);

    // 2. Read current disable flags after CheckUserAccess
    const newDisable = this.accountSetupService.newDisabled.getValue();
    const editDisabled = this.accountSetupService.editDisabled.getValue();
    const deleteDisabled = this.accountSetupService.deleteDisabled.getValue();

    if(this.commonService.isSystemAdmin.value == true){
      this.accountSetupService.newDisabled.next(false); 
      this.accountSetupService.editDisabled.next(false);
      this.accountSetupService.deleteDisabled.next(false);
      return;
    }

    this.accountSetupService.newDisabled.next(!newDisable);
    this.accountSetupService.editDisabled.next(!editDisabled);
    this.accountSetupService.deleteDisabled.next(!deleteDisabled);
