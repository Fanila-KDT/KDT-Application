...
import { PaymentRequestModel } from '../../../../Model/PaymentRequest/payment-request.model';
...
@Component({
  ...
  styleUrls: ['./payment-request-list.css' , '../../../common.css']
})
export class PaymentRequestList {
  ...
  currPage: number = 1;
  selectedPaymentRequest: number=0;
  filterPaymentRequest = new PaymentRequestModel();
  filteredRows: any[] = [];  
  paymentRequestPagingResponse: Pagination = new Pagination();
  startCount:any = 0;
  totalPages : number = 0;
  ...
  constructor(public paymentRequestService:PaymentRequestService,public commonService:CommonService,public endPointService:EndPointService,public userAccessService:UserAccessService,
            private cdRef: ChangeDetectorRef) {
    this.gridHeight = this.endPointService.GridHeight;

    this.subscription.push(this.paymentRequestService.loadList.subscribe(async data => {
      if(data){
        if (data.length !== 0 ) {
          ...
          this.paymentRequestPagingResponse.TotalItems = this.rows.length;
    ...
    this.subscription.push(this.paymentRequestService.disableGrid.subscribe(data=>{
      this.disableGrid = data;
    }));

    this.subscription.push(this.paymentRequestService.ControlsEnableAndDisable.subscribe(data=>{
      if(data){
        this.ControlsEnableAndDisable();
      }
    }));

    this.subscription.push(this.paymentRequestService.ngOnInit.subscribe(data=>{
      if(data){
        ...
        this.paymentRequestService.cancelClick.next(true);
      }
    }));
  ...
  async ngOnInit() {
    this.filterPaymentRequest = new PaymentRequestModel();
    await this.paymentRequestService.GetPaymentRequestList(sessionStorage.getItem('year'),1);
  }
  ...
  ngOnDestroy(): void {
    ...
    if(rowIndex+1!=this.controls.pageSize){
      if (event.type === 'keydown' && (event.event.code === 'ArrowDown'))
      {
        ...
        this.selectedPaymentRequest = 1 + rowIndex;
        this.viewDetails(this.rows[this.selectedPaymentRequest]);
        this.paymentRequestService.clickedPayment.next(this.rows[this.selectedPaymentRequest]);
      } 
      else if (event.type === 'keydown' && (event.event.code === 'ArrowUp'))
      {
        this.selectedPaymentRequest = rowIndex - 1;
        this.viewDetails(this.rows[this.selectedPaymentRequest]);
        this.paymentRequestService.clickedPayment.next(this.rows[this.selectedPaymentRequest]);
      }
      ...
      {
        let rowItem = event.row;
        this.selectedPaymentRequest = this.rows.indexOf(rowItem);
  ...
  updateFilter() {
    const f = this.filterPaymentRequest;
    this.filteredRows = this.allRows.filter(row =>
      (!f.document_number || row.document_number?.toLowerCase().includes(f.document_number.toLowerCase()))&&
      (!f.paid_to || row.paid_to?.toLowerCase().includes(f.paid_to.toLowerCase()))&&
      (!f.reason || row.reason?.toLowerCase().includes(f.reason.toLowerCase()))&&
    ...
    this.paymentRequestService.clickedPayment.next(this.rows[0]);
  }
  ...
  ControlsEnableAndDisable() {
    ...
    this.userAccessService.CheckUserAccess(this.paymentRequestService.FormName, this.paymentRequestService);

    // 2. Read current disable flags after CheckUserAccess
    const newDisable = this.paymentRequestService.newDisabled.getValue();
    const editDisabled = this.paymentRequestService.editDisabled.getValue();
    const deleteDisabled = this.paymentRequestService.deleteDisabled.getValue();

    if(this.commonService.isSystemAdmin.value == true){
      this.paymentRequestService.newDisabled.next(false); 
      this.paymentRequestService.editDisabled.next(false);
      this.paymentRequestService.deleteDisabled.next(false);
      return;
    }

    this.paymentRequestService.newDisabled.next(!newDisable);
    this.paymentRequestService.editDisabled.next(!editDisabled);
    this.paymentRequestService.deleteDisabled.next(!deleteDisabled);
