...
@Component({
  selector: 'purchase-return-list',
  standalone: false,
  templateUrl: './purchase-return-list.html',
  styleUrls: ['./purchase-return-list.css','../../../common.css']
})
export class PurchaseReturnList {
  ...
  constructor(public commonService:CommonService,public goodsRecieptService:PurchaseReturnService,public endPointService:EndPointService,public userAccessService:UserAccessService,
    ...
    this.subscription.push(this.goodsRecieptService.ControlsEnableAndDisable.subscribe(data=>{
      ...
      const hasExpensePermission = this.userAccessList?.some(
        (u: any) => u.permissionName === 'PURCHASE_RETURNS'
      );
    }));
  }
  ...
  async ngOnInit() {
    ...
    await this.goodsRecieptService.getPurchaseReturnList(this.endPointService.year);
    this.cdRef.markForCheck();
  }
  ...
  ControlsEnableAndDisable() {
    ...
    if (row.virtual_store != true && 
        ['DRAFT', 'VERIFICATION FAILED'].includes(row.approval_status?.toUpperCase())) {
      this.goodsRecieptService.SVDisabled.next(false);
    } else {
      this.goodsRecieptService.SVDisabled.next(true);
    }

    const hasExpensePermission = this.userAccessList?.some( (u: any) => u.permissionName === 'PURCHASE_RETURNS' );
    if((this.approval_status).toUpperCase() == 'STOCK VERIFIED' && hasExpensePermission){
