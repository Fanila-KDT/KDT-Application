...
import { StockCorrectionModel } from '../../../../Model/StockCorrection/stock-correction.model';
...
@Component({
  ...
  standalone: false,
  templateUrl: './stock-correction-list.html',
...
subscription: Subscription[] =  new Array<Subscription>();
  ...
  currPage: number = 1;
  selectedStockCorrection: number=0;
  filterStockCorrection = new StockCorrectionModel();
  filteredRows: any[] = [];  
  stockCorrectionPagingResponse: Pagination = new Pagination();
  startCount:any = 0;
  totalPages : number = 0;
  ...
  constructor(public stockCorrectionService:StockCorrectionService,public endPointService:EndPointService,public userAccessService:UserAccessService,
    ...
    this.subscription.push(this.stockCorrectionService.loadListStockCorrection.subscribe(async data => {
      if(data){
        if (data.length !== 0 ) {
          ...
          this.stockCorrectionPagingResponse.TotalItems = this.rows.length;
    ...
    this.subscription.push(this.stockCorrectionService.disableGrid.subscribe(data=>{
    ...
    this.subscription.push(this.stockCorrectionService.ngOnInit.subscribe(data=>{
      if(data){
        ...
        this.stockCorrectionService.cancelClick.next(true);
      }
    }));
  }
  ...
  onActivate(event: any) {
    ...
    if(rowIndex+1!=this.controls.pageSize){
      if (event.type === 'keydown' && (event.event.code === 'ArrowDown'))
      {
        ...
        this.selectedStockCorrection = 1 + rowIndex;
        this.viewDetails(this.rows[this.selectedStockCorrection]);
        this.approval_status = this.rows[this.selectedStockCorrection].approval_status;
        this.stockCorrectionService.clickedStockCorr.next(this.rows[this.selectedStockCorrection]);
      } 
      else if (event.type === 'keydown' && (event.event.code === 'ArrowUp'))
      {
        this.selectedStockCorrection = rowIndex - 1;
        this.viewDetails(this.rows[this.selectedStockCorrection]);
        this.approval_status = this.rows[this.selectedStockCorrection].approval_status;
        this.stockCorrectionService.clickedStockCorr.next(this.rows[this.selectedStockCorrection]);
      }
      ...
      {
        let rowItem = event.row;
        this.selectedStockCorrection = this.rows.indexOf(rowItem);
  ...
  updateFilter() {
    const f = this.filterStockCorrection;
    ...
    this.stockCorrectionService.clickedStockCorr.next(this.rows[0]);
  }
