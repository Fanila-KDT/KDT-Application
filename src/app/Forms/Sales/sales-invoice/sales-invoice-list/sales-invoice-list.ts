...
import { SalesInvoiceModel } from '../../../../Model/SalesInvoice/sales-invoice.model';
...
@Component({
  ...
  styleUrls: ['./sales-invoice-list.css','../../../common.css']
})
export class SalesInvoiceList {
 subscription: Subscription[] =  new Array<Subscription>();
  ...
  currPage: number = 1;
  selectedSalesInvoice: number=0;
  filterSalesInvoice = new SalesInvoiceModel();
  filteredRows: any[] = [];  
  salesInvoicePagingResponse: Pagination = new Pagination();
  startCount:any = 0;
  totalPages : number = 0;
  ...
  endCount: number=0;
   ...
   constructor(public salesInvoiceService:SalesInvoiceService,public commonService:CommonService,public endPointService:EndPointService,public userAccessService:UserAccessService) {
    this.gridHeight = this.endPointService.GridHeight;

    this.subscription.push(this.salesInvoiceService.loadList.subscribe(async data => {
      if(data){
        if (data.length !== 0 ) {
          ...
          this.salesInvoicePagingResponse.TotalItems = this.rows.length;
    ...
    this.subscription.push(this.salesInvoiceService.disableGrid.subscribe(data=>{
      this.disableGrid = data;
    }));

    this.subscription.push(this.salesInvoiceService.ControlsEnableAndDisable.subscribe(data=>{
      if(data){
        this.ControlsEnableAndDisable();
      }
    }));

    this.subscription.push(this.salesInvoiceService.ngOnInit.subscribe(data=>{
      if(data){
        this.salesInvoiceService.getSalesInvoiceList(this.endPointService.year,1);
        this.disableGrid = false;
        this.salesInvoiceService.btnClick.next('');
        this.salesInvoiceService.cancelClick.next(true);
      }
    }));
  ...
  async ngOnInit() {
    this.filterSalesInvoice = new SalesInvoiceModel();
    await this.salesInvoiceService.getSalesInvoiceList(this.endPointService.year,1);
  }
  ...
  ngOnDestroy(): void {
    ...
    if(rowIndex+1!=this.controls.pageSize){
      if (event.type === 'keydown' && (event.event.code === 'ArrowDown'))
      {
        ...
        this.selectedSalesInvoice = 1 + rowIndex;
        this.viewDetails(this.rows[this.selectedSalesInvoice]);
        this.salesInvoiceService.clickedSalesInvoice.next(this.rows[this.selectedSalesInvoice]);
      } 
      else if (event.type === 'keydown' && (event.event.code === 'ArrowUp'))
      {
        this.selectedSalesInvoice = rowIndex - 1;
        this.viewDetails(this.rows[this.selectedSalesInvoice]);
        this.salesInvoiceService.clickedSalesInvoice.next(this.rows[this.selectedSalesInvoice]);
      }
      ...
      {
        let rowItem = event.row;
        this.selectedSalesInvoice = this.rows.indexOf(rowItem);
  ...
  updateFilter() {
    const f = this.filterSalesInvoice;
    ...
    this.salesInvoiceService.clickedSalesInvoice.next(this.rows[0]);
  }
  ...
  ControlsEnableAndDisable() {
    ...
    this.userAccessService.CheckUserAccess(this.salesInvoiceService.FormName, this.salesInvoiceService);

    // 2. Read current disable flags after CheckUserAccess
    const newDisable = this.salesInvoiceService.newDisabled.getValue();
    const editDisabled = this.salesInvoiceService.editDisabled.getValue();
    const deleteDisabled = this.salesInvoiceService.deleteDisabled.getValue();

    if(this.commonService.isSystemAdmin.value == true){
      this.salesInvoiceService.newDisabled.next(false); 
      this.salesInvoiceService.editDisabled.next(false);
      this.salesInvoiceService.deleteDisabled.next(false);
      return;
    }

    this.salesInvoiceService.newDisabled.next(!newDisable);
    this.salesInvoiceService.editDisabled.next(!editDisabled);
    this.salesInvoiceService.deleteDisabled.next(!deleteDisabled);
